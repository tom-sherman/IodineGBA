import XAudioServer from "./XAudioJS/XAudioServer";
import { getFloat32Array } from "../../../IodineGBA/includes/TypedArrayShim";
import Resampler from "./XAudioJS/resampler";
...
GlueCodeMixer.prototype.initializeBuffer = function () {
  this.buffer = new AudioSimpleBuffer(this.channelCount, this.bufferAmount);
};
...
GlueCodeMixer.prototype.unregister = function (stackPosition) {
  ...
  this.outputUnitsValid = [];
  for (
    var index = 0, length = this.outputUnits.length;
    index < length;
    ++index
  ) {
    if (this.outputUnits[index]) {
...
GlueCodeMixer.prototype.checkAudio = function () {
  if (this.audio) {
    ...
    for (
      var count = 0, requested = this.findLowestBufferCount();
      count < requested;
      ++count
    ) {
      for (
...
GlueCodeMixer.prototype.findLowestBufferCount = function () {
  var count = 0;
  for (
    var inputIndex = 0, inputCount = this.outputUnitsValid.length;
    inputIndex < inputCount;
    ++inputIndex
  ) {
    var tempCount =
      this.outputUnitsValid[inputIndex].buffer.resampledSamplesLeft();
    if (tempCount > 0) {
      if (count > 0) {
...
GlueCodeMixerInput.prototype.initialize = function (
  channelCount,
  sampleRate,
  bufferAmount,
  heartBeatCallback,
...
GlueCodeMixerInput.prototype.remainingBuffer = function () {
  return (
    ...
    Math.floor(
      (this.mixer.audio.remainingBuffer() * this.sampleRate) /
        this.mixer.sampleRate /
        this.mixer.channelCount
    ) *
      this.mixer.channelCount
...
AudioBufferWrapper.prototype.initialize = function () {
  ...
  this.resampler = new Resampler(
    this.sampleRate,
    this.mixerSampleRate,
    this.mixerChannelCount,
    this.inBuffer
  );
...
AudioBufferWrapper.prototype.copyOld = function (oldBuffer) {
  ...
  while (oldBuffer.resampleBufferStart != oldBuffer.resampleBufferEnd) {
    this.outBuffer[this.resampleBufferEnd++] =
      oldBuffer.outBuffer[oldBuffer.resampleBufferStart++];
...
AudioBufferWrapper.prototype.push = function (buffer, start, end) {
  ...
  if (this.channelCount < this.mixerChannelCount) {
    for (; start < length && this.inputOffset < this.inBufferSize; ) {
      for (
        var index = this.channelCount;
        index < this.mixerChannelCount;
        ++index
      ) {
    ...
    for (; start < length && this.inputOffset < this.inBufferSize; ) {
      for (
        index = 0;
        index < this.mixerChannelCount && start < length;
        ++index
      ) {
        this.inBuffer[this.inputOffset++] = buffer[start++];
      }
...
AudioBufferWrapper.prototype.setBufferSpace = function (bufferAmount) {
  while (
    this.inputOffset < bufferAmount &&
    this.inputOffset < this.inBufferSize
  ) {
...
AudioBufferWrapper.prototype.remainingBuffer = function () {
  return (
    Math.floor(
      (this.resampledSamplesLeft() * this.resampler.ratioWeight) /
        this.mixerChannelCount
    ) *
      this.mixerChannelCount +
    this.inputOffset
