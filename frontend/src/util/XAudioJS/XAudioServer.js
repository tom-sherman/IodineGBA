...
export default function XAudioServer(
  channels,
  sampleRate,
  minBufferSize,
  maxBufferSize,
  ...
  XAudioJSCallbackAPIEventNotificationCallback =
    typeof heartbeatCallback == "function" ? heartbeatCallback : null;
  XAudioJSCallbackAPIEventNotificationCallback2 =
    typeof postheartbeatCallback == "function" ? postheartbeatCallback : null;
  ...
  this.userEventLatch =
    typeof userEventLatch == "object" ? userEventLatch : null;
  ...
  for (var bufferIndex = 0; bufferIndex < bufferLength; ) {
    ...
    if (resampleLength > 0) {
      ...
      this.samplesAlreadyWritten +=
        this.audioHandleMoz.mozWriteAudio(resampledBuffer);
    }
  }
};
XAudioServer.prototype.callbackBasedWriteAudioNoCallback = function (
  ...
  for (
    var bufferCounter = 0;
    bufferCounter < bufferLength &&
    XAudioJSAudioBufferSize < XAudioJSMaxBufferSize;
...
XAudioServer.prototype.initializeWebAudio = function () {
  if (
    ...
    typeof XAudioJSWebAudioContextHandle == "undefined"
  ) {
    throw null;
  } else {
    if (!this.userEventLatch) {
      ...
      this.userEventLatch.addEventListener(
        "click",
        function () {
...
var XAudioJSCallbackAPIEventNotificationCallbackCompatTimer = setInterval(
